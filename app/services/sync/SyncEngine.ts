...
import { getCurrentUserId, getDeviceId } from "@/services/sync/identity"
...
export async function runSync() {
  ...
  await executeTransaction(db, async (txDb) => {
    ...
    await execute(
      ...
      [SYNC_STATE_ID, response.newCursor, now],
    )
  })
...
function mapChangeLogToOp(op: ChangeLogEntry) {
  return {
    opId: op.opId,
    entityType: op.entityType as "task" | "comment",
...
async function applyRemoteChange(db: SqliteDb, change: SyncChange) {
  ...
  if (hasPending) {
    ...
    const localRevision = local?.revision ?? ""
    if (localRevision !== change.revision) {
...
async function createConflict(
  db: SqliteDb,
  change: SyncChange,
  localPayload?: Task | Comment | null,
) {
  const resolvedLocal = localPayload ?? (change.entityType === "task"
    ? await getTaskById(change.entityId)
    : await getCommentById(change.entityId))

  const localRevision = (resolvedLocal as Task | Comment | null)?.revision ?? ""
