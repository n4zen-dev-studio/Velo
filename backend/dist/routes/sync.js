...
async function applyTaskOp(tx, userId, op) {
...
async function createTaskEventForOp(tx, params) {
    ...
    let payload = { message: "Task updated" };
    if (opType === "DELETE") {
        type = "TASK_DELETED";
    ...
    await logChangeForWorkspace(tx, actorUserId, workspaceId, "task_events", eventId, "UPSERT", {
        id: eventId,
        taskId,
        type,
...
async function applyCommentOp(tx, userId, op) {
    const patch = op.patch;
    const now = new Date();
    const revision = `srv-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
    if (op.opType === "DELETE") {
...
async function applyUserOp(tx, userId, op) {
    const patch = op.patch;
    const now = new Date();
...
async function applyWorkspaceMemberOp(tx, userId, op) {
    const patch = op.patch;
    const now = new Date();
    const revision = patch.revision ?? `srv-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
    ...
    const recipientIds = new Set([userId, record.userId]);
    await logChangeForRecipients(tx, Array.from(recipientIds), "workspace_member", op.entityId, op.opType, record, revision, now);
    await emitWorkspaceIndexChangeForRecipients(tx, record.workspaceId, Array.from(recipientIds), now, patch.workspaceLabel);
    if (op.opType === "UPSERT" && !record.deletedAt && !existingActiveMember) {
        const tasks = await tx.task.findMany({
            where: { workspaceId: record.workspaceId, deletedAt: null },
        });
        for (const task of tasks) {
            await logChangeForRecipients(tx, [record.userId], "task", task.id, "UPSERT", {
                id: task.id,
            ...
            }, task.revision ?? `srv-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`, now);
        }
    }
}
async function logChange(tx, userId, entityType, entityId, opType, payload, revision, updatedAt) {
...
async function resolveWorkspaceIdForTask(tx, taskId) {
    const task = await tx.task.findUnique({ where: { id: taskId }, select: { workspaceId: true } });
    return task?.workspaceId ?? null;
}
async function listWorkspaceRecipientUserIds(tx, workspaceId) {
...
async function logChangeForWorkspace(tx, actorUserId, workspaceId, entityType, entityId, opType, payload, revision, updatedAt) {
    if (!workspaceId) {
...
async function logChangeForRecipients(tx, userIds, entityType, entityId, opType, payload, revision, updatedAt) {
...
async function emitWorkspaceIndexChangeForRecipients(tx, workspaceId, recipientUserIds, now, workspaceLabel) {
...
async function validateTaskOp(tx, userId, op) {
    const patch = op.patch;
    if (op.opType === "UPSERT") {
...
function validateUserOp(_op) {
    return { ok: true, message: "" };
}
function validateWorkspaceMemberOp(op) {
    if (op.opType === "UPSERT") {
        const patch = op.patch;
